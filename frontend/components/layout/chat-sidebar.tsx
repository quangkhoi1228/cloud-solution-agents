...
export function ChatSidebar() {
  const { messages, currentMessage, setCurrentMessage, sendMessage } = useAppStore()
  ...
  return (
    <div ref={containerRef} className="w-80 bg-gray-800 border-r border-gray-700 flex flex-col h-full">
          ...
          {messages.map((msg) => (
            <div key={msg.id} className={`flex gap-3 ${msg.type === "user" ? "flex-row-reverse" : "flex-row"}`}>
              <Avatar className="w-6 h-6 mt-1 flex-shrink-0">
                <AvatarFallback className={`${msg.roleColor || "bg-gray-600"} text-white text-xs`}>
                  {msg.avatar}
                </AvatarFallback>
              </Avatar>
              <div className={`flex-1 ${msg.type === "user" ? "text-right" : "text-left"}`}>
                ...
                <div
                  ...
                  ${msg.type === "user" ? "bg-blue-600 text-white ml-auto" : "bg-gray-700 text-gray-300"}
                `}
                >
